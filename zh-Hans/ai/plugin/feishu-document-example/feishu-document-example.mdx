...
本篇展示一个完整的插件示例，基于 [飞书](https://www.feishu.cn/) 平台实现。
示例包含插件功能、配置、代码结构、接口实现、增强点等内容，帮助你理解如何在 **Xpert AI 平台** 开发类似的集成插件。
...
插件实际是一个 [NestJS 模块](https://docs.nestjs.com/modules)（`LarkModule`），它将不同的 **策略类（Strategy）、控制器（Controller）或者服务（Service）** 注册进来。
...
[控制器](https://docs.nestjs.com/controllers) 用于暴露插件的 REST 接口。例如：测试连接飞书。
...
[系统集成策略 `LarkIntegrationStrategy`](./integration/) 用于处理飞书应用凭证验证，获取机器人信息等。
...
[文档源策略 `LarkSourceStrategy`](./source/) 定义如何从飞书中获取文档列表。
...
[文档转换策略 `LarkDocTransformerStrategy`](./transformer/) 定义如何将文档内容转化为知识库条目。
...
为了确保 npm 包的迭代规范，需要正确维护插件版本号。
推荐使用 Nx 提供的 **release 流程** 自动管理版本：
...
> 如果不使用 `nx release`，也可以手动修改 `package.json` 的 `version` 字段。
...
* `--access public`
  指定包为公开发布（适用于 `@xpert-ai` 这种 npm scope 下的包）。
* `--otp=<code>`
